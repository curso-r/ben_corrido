...
```{r}
benCorrido::tab_consumo_final_setor(con)
```

```{r}
# linha <- benCorrido::grafico_consumo_final_fonte_linha(con, .tipo = "Absoluto")
# area <- benCorrido::grafico_consumo_final_fonte_area(con, .tipo = "Absoluto")
...
```{r}
# linha <- benCorrido::grafico_consumo_final_fonte_linha(con, .tipo = "Percentual")
# area <- benCorrido::grafico_consumo_final_fonte_area(con, .tipo = "Percentual")
...
```{r}
benCorrido::tab_consumo_final_energetico_fonte(con)
```
...
```{r}
benCorrido::tab_consumo_final_nao_energetico_fonte(con)
```
...
```{r}
benCorrido::tab_dependencia_externa_energia(con)
```
...
```{r}
benCorrido::tab_composicao_setorial_consumo_derivados_petroleo(con)
```
...
```{r}
benCorrido::tab_composicao_setorial_consumo_eletricidade(con)
```
...
```{r}
benCorrido::tab_composicao_setorial_consumo_carvao_vapor(con)
```
...
```{r}
benCorrido::tab_composicao_setorial_consumo_final_biomassa(con)
```
...
```{r}
benCorrido::tab_oferta_interna_energia_2(con)
```
