...
grafico_variacao_energia_final_util_potencial <- function(con, lang = "pt") {
  ...
  tab |>
    ggplot2::ggplot(ggplot2::aes(x = ano, y = valor, fill = variavel)) +
    ggplot2::geom_area(color = "#404040", size = 0.25) +
    ggplot2::scale_fill_manual(values = cores) +
    ggplot2::theme_minimal() +
    ggplot2::scale_x_continuous(
      expand = c(0, 0),
      breaks = seq(1984, 2004, by = 10),
      limits = c(1984, 2004)
    ) +
    ggplot2::guides(fill = ggplot2::guide_legend(byrow = TRUE, keywidth = 0.5, keyheight = 0.5)) +
    ggplot2::theme(
    ...
    ggplot2::geom_vline(xintercept = 1984, color = "black", linetype = "solid", size = 0.5) +
    ggplot2::geom_hline(yintercept = 0, color = "black", linetype = "solid", size = 0.35) +
    ggplot2::ylab(rotulo_y)
}
